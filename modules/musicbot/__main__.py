import asyncio
import os
import sys

import discord
from discord.ext import commands

from config import config
from modules.musicbot.bot import MusicBot
from modules.musicbot.utils import OutputWrapper, check_dependencies
from modules.yandex_m_client import YandexMusicClient

initial_extensions = [
    "modules.musicbot.commands.music",
    "modules.musicbot.commands.general",
]


intents = discord.Intents.default()
if config.BOT_PREFIX:
    intents.message_content = True
    prefix = config.BOT_PREFIX
else:
    config.BOT_PREFIX = config.actual_prefix
    prefix = " "  # messages can't start with space
if config.MENTION_AS_PREFIX:
    prefix = commands.when_mentioned_or(prefix)

if config.ENABLE_BUTTON_PLUGIN:
    intents.message_content = True
    initial_extensions.append("modules.musicbot.plugins.button")

bot = MusicBot(
    command_prefix=prefix,
    case_insensitive=True,
    status=discord.Status.online,
    activity=discord.Game(name="Music, type {}help".format(config.BOT_PREFIX)),
    intents=intents,
    allowed_mentions=discord.AllowedMentions.none(),
)


if __name__ == "__main__":

    config.ABSOLUTE_PATH = os.path.dirname(os.path.abspath(__file__))
    config.COOKIE_PATH = config.ABSOLUTE_PATH + config.COOKIE_PATH

    sys.stdout = OutputWrapper(sys.stdout)
    sys.stderr = OutputWrapper(sys.stderr)

    if "--run" in sys.argv:
        print(os.getpid())

    check_dependencies()

    if not config.BOT_TOKEN:
        print("Error: No bot token!")
        exit()

    bot.load_extensions(*initial_extensions)

    asyncio.run(YandexMusicClient.init())

    bot.run(config.BOT_TOKEN, reconnect=True)
