...
from modules.musicbot import linkutils, utils
from modules.musicbot.playlist import Playlist
from modules.musicbot.songinfo import Song
from modules.musicbot.utils import compare_components
...
class AudioController(object):
    ...
    async def process_playlist(self, playlist_type: linkutils.Playlist_Types, url: str):
        ...
        if playlist_type == linkutils.Playlist_Types.Yandex_Album:
            r = await YandexMusicClient.get_album(url)

            for volume in r['volumes']:
                for track in volume:
                    link = url + "/track/" + track['id']
    ...
    async def preload(self, song: Song):
        ...
        elif song.host == linkutils.Sites.Yandex:
            y_data = await YandexMusicClient.get_track(song.info.webpage_url)
